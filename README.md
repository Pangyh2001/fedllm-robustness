# FedEAT-Tail：联邦大模型的平均、尾部与 Clean 鲁棒性
...
1. 客户端从同一个全局 LoRA 状态出发，分别计算 clean 更新和 FedEAT 更新。
...
- `qfedavg_eat`：按照 q-FedAvg 的动态步长公式聚合 FedEAT 更新。
- `fedeat`：本地 embedding 对抗训练、clean loss 和一致性正则 + FedAvg。
- `fedeat_tail`：完整方法。
...
- clean performance 必须与 FedAvg 和 FedEAT-base 同时比较。
